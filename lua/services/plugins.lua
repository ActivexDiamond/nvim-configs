...
--					"*.luarc.*", "LICENSE*", "README*", "*.buildpath", "*.project", "*.travis.*"
			...
			commands = {
				...
				--FIXME: Doesn't work with my current toggleterm setup.
				...
				run_command_on_node_path = function(state)
					local node = state.tree:get_node()
					vim.api.nvim_input(": " .. node:get_id() .. "<Home>")
				end,
				run_command_on_node_name = function(state)
					local node = state.tree:get_node()
					vim.api.nvim_input(": " .. node:get_id() .. "<Home>")
				end,
			},
			window = {
				...
				popup = {       -- settings that apply to float position only
					...
					position = "50%", --Will center the flaot.
				},
				mappings = {
					["Z"] = "expand_all_nodes",
					["s"] = "system_open",
					...
					["q"] = "open_in_terminal",
					["<space>"] = "open",
					["u"] = "run_command_on_node_path",
					["U"] = "run_command_on_node_name",
					--Also, ["b"] to edit base name
					--["s"] to open in system viewer (whther file explorer, editor, etc...)
				},
			},
			follow_current_file = {
				enable = true,
...
M.otter = U.Service({ { FT.CONF, 'otter.nvim' } }, {}, function()
	...
	vim.api.nvim_create_autocmd({ "BufEnter" }, {
		...
		callback = function()
			...
			vim.api.nvim_buf_set_keymap(0, 'n', '<leader>d', ":lua require 'otter'.ask_definition()<cr>",
				{ silent = true })
...
M.edgy = U.Service({ { FT.CONF, 'edgy.nvim' } }, {}, function()
	require 'edgy'.setup {
		bottom = {
			{ ft = "toggleterm", size = { height = 0.8 }, filter = function(buf, win) return vim.api.nvim_win_get_config(
				win).relative == "" end },
